...
local function send_ping()  
    m:publish(config.ENDPOINTtemp .. "ping","id=" .. config.ID,0,0)
end
...
local function register_myself()  
        ...
        m:publish(config.ENDPOINTtemp,measureTemp(),0,0, function(conn)
        end)

        m:publish(config.ENDPOINTres,adc2r(currMeasure,currMUX),0,0, function(conn)
        end)

        m:publish(config.ENDPOINTadc,currMeasure,0,0, function(conn)
        end)

        m:publish(config.ENDPOINTmux,currMUX,0,0, function(conn)
        end)
end
