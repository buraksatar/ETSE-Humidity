         ...
         connection.query('SELECT * FROM chipID;', 
            function (error, rows) 
            {
                ...
                if (checkValue != 0){
                    ...
                    connection.query('DELETE FROM registeredClients WHERE clientName = "'+ dataFromForm.number + rows[checkIndex].queue +'"' ,
                        function (err, result) {
                            if(err) {
                                console.log(err);
                            } 
    ...
    if(dataFromForm.unsubscribe != ''){
         connection.query('SELECT * FROM chipID;', 
            function (error, rows) 
            {
                ...
                // check value cant be zero
	            for(var i=0;i<rows.length;i++){
                    if ( dataFromForm.unsubscribe == tagDatas.chipIDnumber[i] ){
                        checkValue++;
                        checkIndex = i;
                    }
                }
                ...
                if (checkValue != 0){
                    ...
                    //getDatafromRegisteredClientsTable();
                    //console.log('registered data '+countClients.registered);
...
function getDatafromRegisteredClientsTable(){
    ...
    {
        countClients.registered = [];

        for(var i=0;i<rows.length;i++){
            countClients.registered.push(rows[i].clientName);
    	}
    });
}
...
app.get('/configuration', function(req, res) {
	...
	//send the data with those names as an array
        ...
        tagRegistered : countClients.registered
 	});
});
...
app.post('/configuration', function(req, res) {
    ...
    else if ( req.body.numberForUnsubscribe ){
