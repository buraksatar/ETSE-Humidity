...
function readDatafromSensor1(){
    for (var i=0; i<registeredClients.subscriptionName.length ; i++){
        console.log(registeredClients.subscriptionName[i]); 
        
        connection.query('SELECT * FROM ' + tagDatas.subscriptionName[i], 
            function (error, rows) 
            {
                if (error) throw error;
                datas.temp = [];
                ...
                for (var j=0;j<rows.length;j++){
                    datas.temp.push(rows[j].temp);
                    console.log(datas.temp[j]);
                }
            }
        );
    }   
